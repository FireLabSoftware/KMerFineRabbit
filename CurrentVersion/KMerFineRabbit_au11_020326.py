...
##  -   No spaces should be present in the list and if a list is provided the list needs to be in quotes (e.g. Substrate='myRun1_R1,myRun2_R1,myRun3_R1')
##  -   Pairs of "Substrate" files (e.g.,R1,R2) with paired reads should be separated by a semicolon (not comma) and the list must be quoted
##  -        (e.g. Substrate='myRun10_R1;myRun10_R2,myRun20_R1;myRun20_R2') 
##  -   Filter filenames that are also in the Substrate File list will be ignored (not used for filtering)
...
##  -   Positive: Keep only reads in SubstrateFiles with at least one kmer from FilterFiles.
##  -   Negative: Keep only reads in SubstrateFiles with no kmers from FilterFiles.
##
## ->Optional Parameters
...
##  - MinReadLen=<default = klen1> : Instructs KFR to output only reads of this length or greater. For paired reads, both must be at least this long.  
...
##  - MultiLineFastASubstrate = <True/False>.  Setting this to true treats inputs substrate files as multiline FastA files (somewhat slower, but deals -- 'on the fly' with genome or assembly files
...
MultiLineFastASubstrate1 = False ## Setting this to true will allow handling of substrate fasta files where individual entries are on multiple lines
...
Delimiter1 = '\r'         ## Default line delimeter to allow files to be read on Linux/Mac/Windows (or all).  This seems to work.  Setting to '' will default to the format of the input files
...
for a0 in sys.argv[1:]:
    ...
    if s1.lower().startswith('substrate'):
        SubstrateFiles1 =  v1
    elif s1.lower().startswith('filterscale'):
        FilterScale1 =  int(v1)
    elif s1.lower().startswith('trimonthefly') or s1.lower().startswith('filteronthefly'):
    ...
    elif s1.lower().startswith('multilinefastasubstrate'):
        if v1[0].lower() in 'p+ikt':
            MultiLineFastASubstrate1 = True
        elif v1[0].lower() in 'n-erf':
            MultiLineFastASubstrate1 = False
...
if not(FilterFiles1) or not(SubstrateFiles1) or PositiveMode1=='default':
    print()
    print('--------------------------------------------------------------------------------------------')
    print('  KMerFineRabbit Detected incomplete command line paramter list  ')
    print()
    if not(FilterFiles1):
        print(' *****ERROR: "Filter=" Not specified')        
    if not(SubstrateFiles1):
        print(' *****ERROR: "Substrate=" Not specified')        
    if PositiveMode1=='default':
        print(' *****ERROR: "Mode=" Not specified')        
    print()
    print('  Here are your command line options as they were seen by Rabbit:')
    print('     '+' '.join(sys.argv))
    print()
    print('  If you are receiving this message, ')
    print('    a. Make sure Filter=, Substrate=, Mode= are each specified in command line')
    print('    b. You may have used a semicolon or other separator that was not enclosed in quotes****')
    print('      If you specify a file list, it needs to be enclosed in quotes')
    print("      One set of quotes encloses the entire list, e.g.  Substrate='FileA_R1;FileA_R2,FileB_R1;FileB_R2' ")
    print("      (****some system shells truncate a command line upon encountering a semicolon not within quotes)")
    print()
    print("  Here's a sample Command Line: (for two .fastq file pairs A_R1/A_R2 and B_R1/B_R2 filtered against file Ref.fa" )
    print("   python3 KMerFineRabbitx.py Substrate='A_R1.fastq;A_R2.fastq,B_R1.fastq;B_R2.fastq' Filter='Ref.fa' Mode=Positive")
    print()
    print('--------------------------------------------------------------------------------------------')
    exit()
...
if not 'pypy' in sys.version.lower():
    print(' **Note: KMerFineRabbit runs MUCH FASTER with the "PyPy" version of Python (free @ www.pypy.org)')
print()
if MinReadLen1 == -1:
    MinReadLen1 = klen1

def myOpen1(fn,mode):
    LogNote1('Opening File:',fn, "  mode='"+mode+"'")
    if fn.lower().endswith('.gz'):
        return gzip.open(fn, mode=mode)
    else:
        return open(fn, mode=mode)
def myClose1(f):
    if type(f) in (list,tuple,str):
        return ''
    else:
        LogNote1('Closing File: ',f.name)
        f.close()
class FastAToIter1():
    '''input is a fastA file, output is an iterator that simulates a single-sequence-line FastA Format'''
    def __init__(self,F):
        self.F = F
        self.Fp = chain(F,['>xStopIteration',])
        self.name = F.name
        self.n = next(F).strip()
    def __iter__(self):
        return self
    def __next__(self):
        if self.n:
            myN = self.n
            if myN=='>xStopIteration': raise StopIteration
            self.n = ''
            return myN
        else:
            S = []
    ...
    def close(self):
        self.F.close()
def myOpen2(Fn,checkset=11):
    '''input is a sequence data filename, output is a triple of FileObject,DataCycle,LineEnding'''
    LineEnd = '\r'
    MultiLineFastA = False
    if Fn.lower().endswith('.gz'):
        F = gzip.open(Fn, mode='rt')
    else:
        F = open(Fn, mode='rt')
    if Fn.lower().endswith('.fastq') or Fn.lower().endswith('.fastq.gz') or Fn.lower().split()[0].endswith('.fastq')  or Fn.lower().split()[0].endswith('.fastq.gz'):
        DataCycle = 4
        for i,L in enumerate(F):
            if i==0:
                LineEnd = LineEndings1(L)
                break
    else:
        DataCycle = 2
        for i,L in enumerate(F):
            ...
            if i>checkset:
                break
    F.seek(0)
    if DataCycle==2 and (MultiLineFastA or MultiLineFastASubstrate1):
        return FastAToIter1(F),DataCycle,LineEnd
    else:
        return F,DataCycle,LineEnd
def MultiToSingleLineFastA1(myFileName):
    ''' takes a multiline fastA file as input; returns a memory object that emulates an equivalent single-line FastA file with stripped lines'''
    myFile = myOpen1(myFileName, mode='rt')
    myList = []
    for L in myFile:
        if not(L.strip()): continue
        if L.startswith('>'):
            if myList: myList.append(''.join(mySeq))
            myList.append(L.strip())
            mySeq = []
        else:
            if not(myList):
                LogNote1('Improper FASTA File encountered in Substrate List:',myFileName)
                LogNote1('Processing without that file')
                return []
            mySeq.append(L.strip())
    myList.append(mySeq)
    myFile.close()
    return myList
def MultiLineFastAWarning1(myFile):
    LogNote1('')
    LogNote1('***************************************************************************')
    LogNote1('WARNING!  Your substrate file:',myFile,' appears to be in a multiline format')
    LogNote1('You should be able to fix this by re-running KFR with the command line option')
    LogNote1('     MultiLineFastASubstrate = True')
    LogNote1('***************************************************************************')
    LogNote1('')
...
def findfastqdump(candidate):
    ...
    if NewItems:
        ...
        try:
            open('~/FastQDumpLocation.txt', mode='wt').write(candidate)
        except:
            pass
        return candidate
...
for s11 in SubstrateFiles1.split(','):
    ...
    if not(SubstrateFileList1):
        SubstrateFileList1.append(s11)
if SubstrateFileList1==['history']:
    try:
        History1 = myOpen1(os.path.basename(sys.argv[0]).split('.')[0]+'_recents.txt',mode='rt').readlines()
...
if not(FilterFileList1):
    LogNote1('No files found as filter [you must specify Filter=<fasta or fastq files>]')
    exit()
...
if OutFileName1 == 'default':
    ...
    OutFileName1 = OutFileName1.split(';')[0].strip('" '+"'")
    OutFileName2 = OutFileName1.split(';')[1].strip('" '+"'")
elif ',' in OutFileName1:
    OutFileName1 = OutFileName1.split(',')[0].strip('" '+"'")
    OutFileName2 = OutFileName1.split(',')[1].strip('" '+"'")
else:
...
if FilterDataList1:    
    for f1,F1 in enumerate(FilterFileList1):
        ...
        if os.path.isfile(F1):
            Fo1,DataCycle1,LineEnding1 = myOpen2(F1)
        else:
            LogNote1('****No file found with the name "',F1,'" . So Rabbit will assume this is a direct sequence specification. If this is not the case, recheck filenames and locations and rerun')
        ...
        if fastq1:
            ...
            MultiLineFastAReference1 = False
            ...
            for j1,L1 in enumerate(chain(Fo1,['>'])):
                if L1[0]=='>':
                    if s1:
                        if len(s1)>1:
                            MultiLineFastAReference1 = True
                        if MultiLineFastAReference1 and j1<100:
                            LogNote1('Started processing ',os.path.basename(F1),'segment',n1) ## Only announce each reference if in multiline mode
                        v1 = 0
                        a1 = 0
                        s11 = ''.join(s1)
                        LS11 = len(s11)
...
if AggregateOn1 or TrimOnTheFly1:
    ...
    for F1 in SubstrateFileList1:
        ...
        for i1,L1 in enumerate(F1o):
            if i1%DataCycle1==1:
                if L1[0]=='>': MultiLineFastAWarning1(os.path.basename(F1))
                v = 0; a = 0
...
if WriteSet1:
    pf1 = myOpen1(WriteSet1, mode='wb')
    pp1 = pickle.Pickler(pf1)
    pp1.dump(S2)
    myClose1(pf1)
    exit()
for pfN0 in FilterSetList1:
    pf0 = myOpen1(pfN0, mode='rb')
...
if MultiplicityFilterFile1:
    ...
    MFF11 = myOpen1(MultiplicityFilterFile1,mode='rt')
    for L1 in chain(MFF11,['>']):
        if L1[0]=='>':
            if s1:
                v1 = 0
                a1 = 0
...
if OutFileName2:
    OutFile2 = myOpen1(OutFileName2, mode='wt')    
else:
    OutFile2 = ''
...
if OutFile2:
    while iter1<=LastIter1:
        ...
        for F1,F2 in zip(SubstrateFileList1[::2],SubstrateFileList1[1::2]):
            ...
            if not(Delimiter1): Delimiter1 = LineEnding1
            ReportLineGranularity1 = DataCycle1*ReportGranularity1
            F2o,DataCycle2,LineEnding2 = myOpen2(F2)
            ...
            for i1,(L1,L2) in enumerate(zip(F1o,F2o)):
                ...
                if i1%DataCycle1 == 1:
                    ...
                    for j1,c1 in enumerate(s1):
                        ...
                        v0 = ((v0&mask1)<<2)+cd1
                        a0 = (a0>>2)+(3-cd1)*ksam1
                        if CheckRange1[j1] and j1>=StartMatching1 and SkipSubstratePreIndex1==(min(v0,a0) in S2):  ## This statement reads out true if exeptions leading to inclusion/exclusion are caught
                    ...
                    if KeepRead1!=PositiveMode1:
                        ...
                        for j2,c2 in enumerate(s2):
                            if c2 == 'N':
                                StartMatching1 = j2+klen1
                            cd2 = BaseL1[ord(c2)]
                            v2 = ((v2&mask1)<<2)+cd2
                            a2 = (a2>>2)+(3-cd2)*ksam1
                            if CheckRange1[j2] and j2>=StartMatching1 and SkipSubstratePreIndex1==(min(v2,a2) in S2):                            
                                KeepRead1 = PositiveMode1
                                break
                if (i1+1)%DataCycle1==0 and KeepRead1:
                            ...
                            if not('N' in idString1):
                                for rA0 in readA1,readA2:
                                    ...
                                    for jA0,cA0 in enumerate(rA0):
                                        if cA0 == 'N':
                                            StartMatching1 = jA0+klen1
                                        bD0 = BaseL1[ord(cA0)]
                                        vA0 = ((vA0&mask1)<<2)+bD0
                                        aA0 = (aA0>>2)+(3-bD0)*ksam1
                                        vA1 = min(vA0,aA0)
                                        if jA0>=StartMatching1:
                    ...
                    if iter1>LastIter1:
                        if TrimOnTheFly1 and readA1!=accu1[1].strip():
                            accu1[0] = accu1[0].strip()+'_trim[:'+str(len(readA1))+']'
                            accu1[1] = readA1
                            if len(accu1)==4:
                                accu1[3] = accu1[3][:len(readA1)]
                        if TrimOnTheFly1 and readA2!=accu2[1].strip():
                            accu2[0] = accu2[0].strip()+'_trim[:'+str(len(readA2))+']'
                            accu2[1] = readA2
                            if len(accu2)==4:
                                accu2[3] = accu2[3][:len(readA2)]
                        if len(accu1[1].strip())>=MinReadLen1 and len(accu2[1].strip())>=MinReadLen1:
                            NK1 += 1
                            OutFile1.write(Delimiter1.join([xx.strip() for xx  in accu1])+Delimiter1)
                            OutFile2.write(Delimiter1.join([xx.strip() for xx  in accu2])+Delimiter1)
                if iter1 <= LastIter1 and DoneAggregating1: break
            myClose1(F1o); myClose1(F2o)
            if iter1 <= LastIter1 and DoneAggregating1: break
    ...
    while iter1<=LastIter1:
        ...
        for F1 in SubstrateFileList1:
            ...
            if not(Delimiter1): Delimiter1 = LineEnding1
            ReportLineGranularity1 = DataCycle1*ReportGranularity1
            myFileMnemonic1 = os.path.basename(F1).split('_')[0].split('.')[0] 
            for i1,L1 in enumerate(F1o):
                if i1%DataCycle1 == 0:
                    if MaxReadsPerFile1 and DataCycle1*MaxReadsPerFile1==i1: break
                    accu1 = []
                ...
                if i1%DataCycle1 == 1:
                    ...
                    for j1,c1 in enumerate(s1):
                        ...
                        v0 = ((v0&mask1)<<2)+cd1
                        a0 = (a0>>2)+(3-cd1)*ksam1
                        if CheckRange1[j1] and j1>=StartMatching1 and SkipSubstratePreIndex1==(min(v0,a0) in S2):
                            KeepRead1 = PositiveMode1
                            break
                if (i1+1)%DataCycle1==0 and KeepRead1:
                    NR1 += 1
...
HistoryFile1.write('\r'+' '.join(sys.argv)+vnow+Delimiter1)
HistoryFile1.write(OutFileName1+','+OutFileName2+Delimiter1)
...
## 03-15-25 version fixes a bug that lost the last k-mer of references (oops) that had appeaered in version au01
