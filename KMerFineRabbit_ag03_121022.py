...
##  -   If filter dictionaries have already been assembled, they can be used as filters by specifying '.pck' files
Threads1 = 1
...
WriteDictionary1 = False  ## Instructs this instance of KFR to write a filter dictionary and stop
MyThread1 = 0             ## For use if KFR is running in multithreaded mode
Turbo1 = False            ## Turbo=True will slightly spees up subtractions with many filter files (~40%)
FilterScale1 = 28         ## FilterScale controls memory buffer used for Turbo mode (30 uses 125MB)

##  - MinKCount=<int> : the minimum number of Kmer occurences in the StartFiles datasets.
##  -   MinKCount1=1 [default] means every K-mer in any "Positive" file is examined,
##  -   MinKCount1=100 means you only examine k-mers appearing at least 100 times in all aggregate positive data
MinKCount1 = 1
...
for a0 in sys.argv[1:]:
    ...
    if s1.lower().startswith('fastq'):
        ...
        if v1.lower()[0] in ('ty1'):
            Turbo1 = True
    elif s1.lower().startswith('writedict'):
        WriteDictionary1 = str(v1)
t0 = time()
...
if OutFileName1 == 'default':
    ...
    if PairedInput1:
        OutFileName1 = FileListMnemonic1(SubstrateFileList1)+pm1+FileListMnemonic1(FilterFileList1)+'_R1_'+vnow+Chaser1
        OutFileName2 = FileListMnemonic1(SubstrateFileList1)+pm1+FileListMnemonic1(FilterFileList1)+'_R2_'+vnow+Chaser1
    else:
...
for F1 in SubstrateFileList1:
    ...
    for i1,L1 in enumerate(F1o):
        ...
        if i1%DataCycle1==1:
            if i1%ReportLineGranularity1==1:
                LogNote1('Substrate File:',os.path.basename(F1),' Read:',i1//DataCycle1, ' Kmers In Primary:', len(C2))
            v0 = 0
            a0 = 0
            for c1 in L1.strip():
                cd1 = BaseD1[c1]
    ...
    F1o.close()
    LogNote1('Finishing Substrate File:',os.path.basename(F1),' Reads:',i1//DataCycle1, ' Kmers In Primary:', len(C2))
if MinKCount1>1:
    for v0 in list(C2.keys()):
        if C2[v0]<MinKCount1:
            del(C2[v0])
for f1 in FilterFileList1:
    if f1.endswith('.pck'):
        FilterDictList1.append(f1)
    else:
...
if FilterDataList1:    
    for f1,F1 in enumerate(FilterFileList1):
            ...
            for j1,L1 in enumerate(Fo1):
                if j1%4==1:
                    if j1%ReportLineGranularity1==1:
                        if Threads1>1: LogNote1('Thread',str(MyThread1+1)+'/'+str(Threads1),'processing',os.path.basename(F1),
                          'at read',j1//4)
                        else:
                    ...
                    for i1,c1 in enumerate(L1.strip()):
                        ...
                        if (i1>=klen1-1):
                            ...
                            if Turbo1:
                                ...
                                APos2 = vamin1&mask3
                                if (A0[APos1] & (1<<APos2)) and vamin1 in C2:
                                    C2[vamin1] = 0
                            else:
                                if vamin1 in C2: C2[vamin1] = 0 
                    NB1 += i1
                    NR1 += 1
        else:
                    ...
                    if s1:
                        ...
                        if MultiLineFastA1:
                            ...
                            if Threads1>1:
                              ...
                              'chromosome',n1,
                              'read',len(i1//ReportLineGranularity1))
                            else:
                                LogNote1('Processing ',os.path.basename(F1),
                              'chromosome',n1,
                              'read',len(i1//ReportLineGranularity1))
        ...
        Fo1.close()
    oldLen1 = len(C2)
    C2 = {c2:C2[c2] for c2 in C2 if C2[c2]>0}
    if Threads1>1:
        LogNote1('Thread',str(MyThread1+1)+'/'+str(Threads1), 'Finished File:',os.path.basename(F1),
          ...
          ' Reads:',NR1,
          ' Bases:',NB1,
          ' Kmer Reduction:',oldLen1,'to',len(C2))
    else:
        LogNote1('Finished File:',os.path.basename(F1),
          ...
          ' Kmer Reduction:',oldLen1,'to',len(C2))
if WriteDictionary1:
    pf1 = open(WriteDictionary1, mode='wb')
    pp1 = pickle.Pickler(pf1)
    pp1.dump(C2)
    pf1.close()
    exit()
for pfN0 in FilterDictList1:
    pf0 = open(pfN0, mode='rb')
    pp0 = pickle.Unpickler(pf0)
    MyD1 = pp0.load()
    pf0.close()
    OldLen1 = len(C2)
    C2 = {c2:C2[c2] for c2 in C2 if c2 in MyD1}
    LogNote1('Applied Dictionary Filter:',os.path.basename(pfN0),' StartKCount:',OldLen1,' FilteredKCount',len(C2))
        ...
        for i1,(L1,L2) in enumerate(zip(F1o,F2o)):
            ...
            if i1%DataCycle1 == 1:
                ...
                for j1,c1 in enumerate(L1.strip()):
                    ...
                    v0 = ((v0&mask1)<<2)+cd1
                    a0 = (a0>>2)+(3-cd1)*ksam1
                    if (j1>=klen1-1) and not(min(v0,a0) in C2):                            
    ...
    for F1 in SubstrateFileList1:
        ...
        for i1,L1 in enumerate(F1o):
            if i1%DataCycle1 == 0:
                ...
                if i1%ReportLineGranularity1==0:
                    LogNote1('Substrate Retention:',os.path.basename(F1),' Read:',i1//DataCycle1, ' KeptReads:', NR1)
            accu1.append(L1)
            if i1%DataCycle1 == 1:
                v0 = 0
                a0 = 0
            ...
            if (i1+1)%DataCycle1==0 and KeepRead1:
                ...
                OutFile1.write(''.join(accu1))
        F1o.close()
LogNote1('Finishing KFR.  Wrote ',NR1,' filtered reads from ',TR1,' total reads, to file ',OutFileName1)
OutFile1.close()
if OutFile2:
